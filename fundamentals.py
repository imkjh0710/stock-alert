"""
펀더멘털 분석 모듈
ROE / EPS 성장 / 영업이익률 / FCF 마진 / PER / PBR 기반 점수 (-17 ~ +17)
...
def _per_score(per) -> int:
    if per is None:
        return 0
    if per <= PER_CHEAP_STRONG:  return  2
    if per <= PER_CHEAP:         return  1
    if per >  PER_PRICEY_STRONG: return -2
    if per >  PER_PRICEY:        return -1
    return 0


def _pbr_score(pbr) -> int:
    if pbr is None:
        return 0
    if pbr <  PBR_CHEAP:         return  1
    if pbr >  PBR_PRICEY_STRONG: return -2
    if pbr >  PBR_PRICEY:        return -1
    return 0
...
def calculate_fundamental_score(info: dict) -> dict:
    """
    yfinance ticker.info → 펀더멘털 점수 (-17 ~ +17).
    ROE/EPS/영업이익률/FCF + PER/PBR 밸류에이션 포함.
    ...
    per_s    = _per_score(per_v)
    pbr_s    = _pbr_score(pbr_v)
    total    = roe_s + eps_s + margin_s + fcf_s + per_s + pbr_s
